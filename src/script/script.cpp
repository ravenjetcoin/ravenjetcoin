...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** MRL START */
    case OP_MRL_ASSET              : return "OP_MRL_ASSET";
    /** MRL END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_MRL_ASSET) { // OP_MRL_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == MRL_R) { // Check to see if MRL starts at 27 ( this->size() < 105)
                if ((*this)[28] == MRL_V)
                    if ((*this)[29] == MRL_N)
                        index = 30;
            } else {
                if ((*this)[28] == MRL_R) // Check to see if MRL starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == MRL_V)
                        if ((*this)[30] == MRL_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == MRL_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == MRL_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == MRL_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == MRL_R) {
