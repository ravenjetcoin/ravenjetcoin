...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of MARLINCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("MRL", type));
        BOOST_CHECK(!IsAssetNameValid("MARLIN", type));
        BOOST_CHECK(!IsAssetNameValid("MARLINCOIN", type));

        //- Versions of MARLINCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("MARLIN.COIN", type));
        BOOST_CHECK(IsAssetNameValid("MARLIN_COIN", type));
        BOOST_CHECK(IsAssetNameValid("MRLSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERMRL", type));
        BOOST_CHECK(IsAssetNameValid("MARLINSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEMARLIN", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_MARLINS", type));
        BOOST_CHECK(IsAssetNameValid("SEMRLOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("MARLIN", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("MARLIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
