...
BOOST_FIXTURE_TEST_SUITE(asset_reissue_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test)
    {
        ...
        CNewAsset asset1("MRLASSET", CAmount(100 * COIN), 8, 1, 0, "");

        // Add an asset to a valid mrl address
        ...
        CReissueAsset reissue1("MRLASSET", CAmount(1 * COIN), 8, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("MRLASSET"), "Map Reissued Asset should contain the asset \"MRLASSET\"");
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("MRLASSET", Params().GlobalBurnAddress())) == CAmount(101 * COIN), "Reissued amount wasn't added to the previous total");
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddresses.at("MRLASSET").count(Params().GlobalBurnAddress()), "Reissued address wasn't in the map");

        // Get the new asset data from the cache
        CNewAsset asset2;
        BOOST_CHECK_MESSAGE(cache.GetAssetMetaDataIfExists("MRLASSET", asset2), "Failed to get the asset2");
        ...
        BOOST_CHECK_MESSAGE(asset2.strName == "MRLASSET", "Asset2: Asset name is wrong");
        ...
        undoBlockData.emplace_back(std::make_pair("MRLASSET", CBlockAssetUndo{true, false, "", 0}));
        ...
        CNewAsset asset3;
        BOOST_CHECK_MESSAGE(cache.GetAssetMetaDataIfExists("MRLASSET", asset3), "Failed to get the asset3");
        ...
        BOOST_CHECK_MESSAGE(asset3.strName == "MRLASSET", "Asset3: Asset name is wrong");
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("MRLASSET"), "Map of reissued data was removed, even though changes were made and not databased yet");
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("MRLASSET", Params().GlobalBurnAddress())) == CAmount(100 * COIN), "Assets total wasn't undone when reissuance was");
    }
    ...
    BOOST_AUTO_TEST_CASE(reissue_isvalid_test)
    {
        ...
        CReissueAsset reissue3("MRLASSET", CAmount(1 * COIN), 7, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueAsset reissue4("MRLASSET", CAmount(1 * COIN), -1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));

        BOOST_CHECK_MESSAGE(reissue4.IsValid(error, cache), "Reissue4 wasn't valid");

        // Create a new asset object with units of 0
        CNewAsset asset2("MRLASSET2", CAmount(100 * COIN), 0, 1, 0, "");

        // Add new asset to a valid mrl address
        ...
        CReissueAsset reissue5("MRLASSET2", CAmount(1 * COIN), 1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueAsset reissue6("MRLASSET2", CAmount(1 * COIN), 1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
