...
const std::string MarlinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MarlinGUI::DEFAULT_WALLET = "~Default";

MarlinGUI::MarlinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void MarlinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Marlin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and marlin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to MRL addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Marlin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Marlin addresses"));
    ...
    openAction->setStatusTip(tr("Open a marlin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Marlin command-line options").arg(tr(PACKAGE_NAME)));
...
void MarlinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/marlincointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* marlinLabelLayout = new QVBoxLayout(toolbarWidget);
        marlinLabelLayout->addWidget(label);
        marlinLabelLayout->addWidget(toolbar);
        marlinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        marlinLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Marlincoin Market Price"));
...
void MarlinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MarlinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MarlinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MarlinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MarlinGUI::setWalletActionsEnabled(bool enabled)
{
...
void MarlinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void MarlinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MarlinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MarlinGUI::showDebugWindow()
{
...
void MarlinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MarlinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MarlinGUI::openClicked()
{
...
void MarlinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MarlinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MarlinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MarlinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MarlinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MarlinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** MRL START */
void MarlinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void MarlinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void MarlinGUI::gotoManageAssetsPage()
{
...
void MarlinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Marlin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MarlinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MarlinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MarlinGUI::updateHeadersSyncProgressLabel()
{
...
void MarlinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void MarlinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Marlin"); // default title
...
void MarlinGUI::changeEvent(QEvent *e)
{
...
void MarlinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MarlinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MarlinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "MRL")
        msg += tr("Amount: %1\n").arg(MarlinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(MarlinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void MarlinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MarlinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MarlinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MarlinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MarlinGUI::setHDStatus(int hdEnabled)
{
...
void MarlinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MarlinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MarlinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MarlinGUI::detectShutdown()
{
...
void MarlinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MarlinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MarlinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MarlinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void MarlinGUI::subscribeToCoreSignals()
{
...
void MarlinGUI::unsubscribeFromCoreSignals()
{
...
void MarlinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<MarlinUnits::Unit> units = MarlinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MarlinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(MarlinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (MarlinUnits::Unit u : MarlinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MarlinUnits::name(u)), this);
...
{
    setText(MarlinUnits::name(newUnits));
}
...
void MarlinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=MRLBTC"));
