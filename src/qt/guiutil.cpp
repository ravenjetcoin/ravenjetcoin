...
#include "marlinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Marlin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new MarlinAddressEntryValidator(parent));
    widget->setCheckValidator(new MarlinAddressCheckValidator(parent));
}
...
bool parseMarlinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no marlin: URI
    if(!uri.isValid() || uri.scheme() != QString("marlin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MarlinUnits::parse(MarlinUnits::MRL, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMarlinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert marlin:// to marlin:
    //
    //    Cannot handle this later, because marlin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("marlin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "marlin:");
    }
    QUrl uriInstance(uri);
    return parseMarlinURI(uriInstance, out);
}

QString formatMarlinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("marlin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MarlinUnits::format(MarlinUnits::MRL, info.amount, false, MarlinUnits::separatorNever));
        paramCount++;
    }
...
bool openMarlinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(MARLIN_CONF_FILENAME);
    ...
    /* Open marlin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Marlin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Marlin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Marlin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Marlin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "marlin.desktop";
    return GetAutostartDir() / strprintf("marlin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a marlin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Marlin\n";
        else
            optionFile << strprintf("Name=Marlin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the marlin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef marlinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (marlinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, marlinAppUrl);

    CFRelease(marlinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add marlin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, marlinAppUrl, nullptr, nullptr);
    }
