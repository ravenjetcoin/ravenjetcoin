...
The RPM spec file provided here is for Marlin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://marlin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Marlin git repository](https://github.com/MarlinProject/Marlincoin) but are not part of
...
Currently only `Source10` is used, for the example `marlin.conf` file.
...
in the marlin git repository. At present only an SVG version of the Marlin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the marlin
...
needed when 0.12.1 is released, a proper fix is already in the Marlin git
master branch.
...
idea to build Marlin Core against a static version of that library compiled
...
The `marlin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `marlin-server` package. At this point, most RPM based
...
The `marlin-server` package should have SELinux support. How to properly do
...
Marlin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/marlin/). He has been
packaging Marlin for Fedora at least since 2012.
...
4. I renamed the `marlin` package that contains the Qt GUI to `marlin-core` as
that appears to be how the general population refers to it, in contrast to
`marlin-xt` or `marlin-classic`. I wanted to make sure the general population
